...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const descricao =
    produto.descricao ?? `${produto.nome}${produto.marca ? `, ${produto.marca}` : ""}. Na Mimos da Mah, a partir de R$10.`;
  ...
  return {
    ...
    openGraph: {
      type: "website",
      title: `${produto.nome} · Mimos da Mah`,
      description: descricao,
...
export default async function PaginaProduto({ params }: Props) {
  ...
  const jsonLd = {
    ...
    offers: {
      ...
      ...(preco ? { price: preco } : {}),
      seller: { "@type": "Store", name: "Mimos da Mah" },
    },
  };
