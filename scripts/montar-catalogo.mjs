/**
 * MONTAR CATÁLOGO — das fotos colhidas ao data/ da vitrine
 *
 * Lê `data/fonte.json` (nome, categoria, preço, tamanhos, descrição de
 * cada item) e as fotos de `_fonte/mimos/<slug>.jpg` que o
 * colher-fotos.mjs baixou, e escreve o que a base lê:
 * `data/catalogo.json` e `public/produtos/{slug}.webp`, com o blur.
 * Item sem foto na pasta fica de fora (e é avisado).
 *
 *   node scripts/colher-fotos.mjs && node scripts/montar-catalogo.mjs
 */
...
for (const item of fonte.itens) {
  ...
  produtos.push({
    ...
    codigo: `MM-${String(produtos.length + 1).padStart(4, "0")}`,
    nome: item.nome,
    slug,
