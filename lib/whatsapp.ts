...
export function linkPeca(produto: Pick<Produto, "codigo" | "nome" | "slug">, opcoes: Pedido = {}): string {
  ...
  const linhas = [
    "Oi! Vi no site da Mimos da Mah e quero este mimo:",
    `• ${produto.nome}`,
    `• Quantidade: ${quantidade}`,
    opcoes.tamanho ? `• Cor ou tom: ${opcoes.tamanho}` : null,
...
export function linkGeral(whatsapp?: string): string {
  return linkWhatsApp("Oi! Vim pelo site da Mimos da Mah.", whatsapp);
}
