...
export function CompraProduto({ produto, whatsapp, base }: { produto: Produto; whatsapp: string; base: string; atendimento?: boolean }) {
  ...
  const pedeTamanho = !["perfumaria", "pinceis-e-acessorios"].includes(produto.categoria_slug ?? "");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-5 sm:grid-cols-[auto_minmax(0,1fr)] sm:items-end">
        ...
        {pedeTamanho && !produto.tamanhos.length ? (
          <label className="campo-flutuante">
            <span>Cor ou tom</span>
            <input value={tamanhoLivre} onChange={(e) => setTamanhoLivre(e.target.value)} placeholder="Ex.: rosa, nude, o mais escuro" maxLength={20} />
          </label>
        ) : null}
      </div>
      ...
      <div className="rounded-[var(--raio)] border border-linha p-5 sm:p-6">
        ...
        <p className="miudo mt-3">A mensagem já vai com o produto, a quantidade e o que você escolheu. A loja confirma se tem e combina a entrega.</p>
      </div>
    </div>
  );
}
